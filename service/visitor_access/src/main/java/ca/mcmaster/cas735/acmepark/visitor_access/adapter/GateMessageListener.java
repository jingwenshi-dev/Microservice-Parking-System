...
public class GateMessageListener {
    ...
    @RabbitListener(bindings = @QueueBinding(value = @Queue(value = "gateEntryResponseQueue", durable = "true"), // 定义队列名称为 gateEntryResponseQueue，并设置为持久化
            exchange = @Exchange(value = "${app.custom.messaging.visitor-to-gate-entry-response-exchange}",
    ...
    public void listenForGateEntryResponse(String data) {
        ...
        gateInteractionServiceImpl.handleGateEntryResponse(data);
    }
    ...
    public void listenForGateExitResponse(String data) {
        ...
        // 调用 VisitorService 来处理 Gate 返回的结果
        gateInteractionServiceImpl.handleGateExitResponse(data);
    }
}
